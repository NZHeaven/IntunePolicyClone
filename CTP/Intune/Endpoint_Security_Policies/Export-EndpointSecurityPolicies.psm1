...
Function Format-Endpoint_Detection_And_Response_Config() {
    param (
        $JSON
    )
    ...
    foreach ($setting in $JSON.settingsDelta) {
        if ($setting.value -eq "True") {
            $Correct_Settings += $setting
        }
    }
    $JSON.settingsDelta = $Correct_Settings
...
Function Export-EndpointSecurityPolicies() {
    ...
    foreach ($policy in $ESPolicies) {
        ...
        # Clean Endpoint Detection and Response JSON
        if ($TemplateId -eq "e44c2ca3-2f9a-400a-a113-6cc88efd773d") {
            Write-Host "Cleaning Endpoint Detection and Response policy" -ForegroundColor Yellow
            $JSON = Format-Endpoint_Detection_And_Response_Config -JSON $JSON
        }

        ####################################################
        Export-JSONData -JSON $JSON -ExportPath "$FolderPath"
...
Export-ModuleMember -Function  Export-EndpointSecurityPolicies